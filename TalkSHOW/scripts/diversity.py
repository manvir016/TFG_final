...
def infer(data_root, g_body, g_face, g_body2, exp_name, infer_loader, infer_set, device, norm_stats, smplx,
    ...
    for bat in infer_loader:
        ...
        if poses_.shape[-1] == 300:
            ...
            speech_array, _ = librosa.load(cur_wav_file, sr=16000)

            # Añadir ruido dinámicamente
            noise_prob = 0.5
            ...
            if random.random() < noise_prob and len(noise_files) > 0:
                ...
                tmp_dir = os.path.join("demo_audio", "tmp_renders")
                os.makedirs(tmp_dir, exist_ok=True)
                tmp_audio_path = os.path.join(tmp_dir, f"augmented_{uuid.uuid4().hex[:6]}.wav")
                sf.write(tmp_audio_path, speech_array, samplerate=16000)

                print(f"[INFO] Ruido añadido: {os.path.basename(noise_file)} → {tmp_audio_path}")
                cur_wav_file = tmp_audio_path  # Reemplaza el .wav que usará el modelo
