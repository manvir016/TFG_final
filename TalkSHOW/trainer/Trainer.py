...
class Trainer():
    def __init__(self) -> None:
        ...
        #W&B
        run_name = self.args.wandb_run_name or f"{self.args.exp_name}-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
        ...
        #set true while doing finetuning, false to simple training
        self.use_mfcc_dataset = False
    ...
    def config_to_dict(self):
        try:
            return {
                "batch_size": getattr(self.config.DataLoader, "batch_size", None),
                "num_workers": getattr(self.config.DataLoader, "num_workers", None),
                "epochs": getattr(self.config.Train, "epochs", None),
                "learning_rate_generator": getattr(getattr(self.config.Train, "learning_rate", {}), "generator_learning_rate", None),
                "learning_rate_discriminator": getattr(getattr(self.config.Train, "learning_rate", {}), "discriminator_learning_rate", None),
                "feat_method": getattr(self.config.Data.aud, "feat_method", None),
                "aud_feat_dim": getattr(self.config.Data.aud, "aud_feat_dim", None),
                "model_name": getattr(self.config.Model, "model_name", None),
                "encoder_choice": getattr(self.config.Model, "encoder_choice", None),
            }   
        except Exception as e:
            print(f"[WARNING] Error parsing config to dict: {e}")
            return {}
    ...
    def resume(self):
        ...
        self.start_epoch = 0 #for finetuning
        self.global_steps = ckpt['global_steps']
        self.generator.global_step = self.global_steps


    def init_dataloader(self):
        if self.use_mfcc_dataset:
            from dataloader_mfcc import create_mfcc_dataloader
            self.train_loader = create_mfcc_dataloader(
                mfcc_folder="mfcc_dataset",
            ...
            if 'freeMo' in self.config.Model.model_name:
                ...
                # ✨ If robust training (FakeSample dataset), build manually
                if hasattr(self.train_set, 'all_dataset'):
                    ...
                    # Handle special robust fine-tuning (FakeSample loading WAVs)
                    from torch.utils.data import DataLoader

                    self.train_loader = DataLoader(self.train_set.all_dataset,
                                    batch_size=1,  # Or you can use config value
                                    shuffle=True,
                                    num_workers=0,
                                    drop_last=True)
    ...
    def train_epoch(self, epoch):
        print(f"Entrando en train_epoch, epoch {epoch}")  
        ...
        if 'freeMo' in self.config.Model.model_name:
            ...
            #W&B
            if epoch_steps > 0:
                loss_to_log = {f"train/{k}": v / epoch_steps for k, v in epoch_loss_dict.items()}
                loss_to_log["epoch"] = epoch
                print(f"[DEBUG] Logging to wandb: {loss_to_log}")
                wandb.log(loss_to_log, step=self.global_steps)
                

        else:
            # self.config.Model.model_name==smplx_S2G
            for bat in self.train_loader:
                print("Leyendo un batch...")
                # if epoch_steps == 1000:
                #     break
                self.global_steps += 1
                epoch_steps += 1
            ...
            if epoch_steps > 0:
                loss_to_log = {f"train/{k}": v / epoch_steps for k, v in epoch_loss_dict.items()}
                loss_to_log["epoch"] = epoch
                print(f"[DEBUG wandb.log] freeMo -> {loss_to_log}")
                wandb.log(loss_to_log, step=self.global_steps)

    def train(self):
        logging.info('start_training')
        print(f"Número de muestras en train_loader: {len(self.train_loader)}") 
        self.total_loss_dict = {}
        
        print(f"Tipo de self.train_loader:", type(self.train_loader))
        print(f"Número de batches que contiene:", len(self.train_loader))


        print(f"start_epoch:", self.start_epoch)
        print(f"total_epochs:", self.config.Train.epochs)


        for epoch in range(self.start_epoch, self.config.Train.epochs):
            print(f"Comenzando epoch {epoch}...")
