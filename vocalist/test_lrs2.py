...
parser.add_argument("--data_root", help="Root folder of the preprocessed LRS2 dataset",
                    default="/home/mkaur/tfg/prueba2/vocalist/videos")
parser.add_argument('--checkpoint_path', help='Resumed from this checkpoint',
                    default="/home/mkaur/tfg/prueba2/vocalist/vocalist_5f_lrs2.pth",
                    type=str)
...
def eval_model(test_data_loader, device, model):
    ...
    for step, (vid, aud, lastframe) in prog_bar:
        ...
        with torch.no_grad():
            ...
            # Obtener el nombre del vídeo actual
            video_name = os.path.basename(test_data_loader.dataset.all_videos[step])

            # Guardar resultados en JSON
            result_entry = {
                "video": video_name,
                "K5": float(np.mean(samplewise_acc_k5)),
                "K7": float(np.mean(samplewise_acc_k7)),
                "K9": float(np.mean(samplewise_acc_k9)),
                "K11": float(np.mean(samplewise_acc_k11)),
                "K13": float(np.mean(samplewise_acc_k13)),
                "K15": float(np.mean(samplewise_acc_k15))
            }       

            # Ruta del archivo donde acumular los resultados
            results_path = "results/results_per_video.json"

            # Si existe, cargar y añadir, si no, crear nuevo
